...
public class BundleTooltipHandler {
    ...
    private int gridSizeX() {
        return VanillaBackport.CLIENT_CONFIG.endlessBundleUi.get()
            ? Math.max(4, Mth.ceil(Math.sqrt(this.items.size())))
            : 4;
    }
    ...
    private void drawProgressBar(int x, int y, Font textRenderer, GuiGraphics graphics) {
        ...
        graphics.blitNineSliced(BUNDLE_PROGRESS_BAR_TEXTURE, x, y, getWidth(), 13, 2, 12, 12, 0, 0);
        
        Component fillText = this.getProgressBarFillText();
        if (fillText != null) {
            graphics.drawCenteredString(textRenderer, fillText, x + (getWidth() / 2), y + 3, -1);
        }
    }
    ...
    private int getProgressBarFill() {
        return Mth.clamp((this.weight * (getWidth() - 2)) / 64, 0, getWidth() - 2);
    }
