...
public record PiercingWeapon(
    ...
    public void attack(LivingEntity attacker, EquipmentSlot hand) {
        ...
        AttackRange attackRange = spearHandler.getAttackRangeWith(weapon);
        ...
        for (EntityHitResult hitResult : hitResults) {
            hitSomething |= spearHandler.stabAttack(hand, hitResult.getEntity(), damage, true, this.dealsKnockback, this.dismounts);
        }
        
        spearHandler.onAttack();
        spearHandler.postPiercingAttack();
        if (hitSomething) {
    ...
    public static boolean hasPiercingWeapon(ItemStack stack) {
        return getPiercingWeapon(stack) != null;
    }
    
    public static @Nullable PiercingWeapon getPiercingWeapon(ItemStack stack) {
