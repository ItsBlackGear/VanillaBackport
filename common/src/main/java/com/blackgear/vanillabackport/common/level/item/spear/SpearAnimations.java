...
public class SpearAnimations {
    ...
    public static <T extends LivingEntity> void thirdPersonUseItem(
        ...
        if (kineticWeapon != null && timeHeld != 0.0F) {
            ...
            float hitFeedback = hitFeedbackAmount(((MobSpearHandler)entity).getTicksSinceLastKineticHitFeedback(partial));
    ...
    public static <T extends LivingEntity> void thirdPersonAttackItem(
        ...
        if (!(attackTime <= 0.0F)) {
            KineticWeapon kineticWeapon = entity.getMainHandItem().get(ModDataComponents.KINETIC_WEAPON.get());
