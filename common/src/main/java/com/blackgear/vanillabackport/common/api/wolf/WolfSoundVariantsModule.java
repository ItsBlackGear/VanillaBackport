...
public record WolfSoundVariantsModule(SynchedEntityData data) {
    ...
    public void defineSynchedData() {
        this.data.define(DATA_SOUND_VARIANT_ID, ModBuiltinRegistries.VARIANTS.getOrThrow(WolfSoundVariants.CLASSIC));
    }

    public void addAdditionalSaveData(CompoundTag tag) {
        tag.putString("sound_variant", ModBuiltinRegistries.VARIANTS.getKey(this.getSoundVariant()).toString());
    }

    public void readAdditionalSaveData(CompoundTag tag) {
        WolfSoundVariant soundVariant = ModBuiltinRegistries.VARIANTS.get(ResourceLocation.tryParse(tag.getString("sound_variant")));
    ...
    public void finalizeSpawn(ServerLevelAccessor level) {
        this.setSoundVariant(ModBuiltinRegistries.VARIANTS.getRandomElement(level.getRandom()));
    }

    public void breedOffspring(Wolf child, Wolf parent) {
        WolfSoundVariantHolder.of(child).setSoundVariant(ModBuiltinRegistries.VARIANTS.getRandomElement(parent.getRandom()));
    }
}