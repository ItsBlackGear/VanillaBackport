...
public class ModDataComponents {
    public static final CoreRegistry<DataComponentType<?>> COMPONENTS = CoreRegistry.create(BuiltInRegistries.DATA_COMPONENT_TYPE, VanillaBackport.NAMESPACE);
    ...
    public static <T> Supplier<DataComponentType<T>> register(String key, UnaryOperator<DataComponentType.Builder<T>> operator) {
        return COMPONENTS.register(key, () -> operator.apply(DataComponentType.builder()).build());
    }
}