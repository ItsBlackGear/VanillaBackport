...
public class ModTrimMaterials {
    ...
    private static ResourceKey<TrimMaterial> register(String name, Item ingredient, Style style, float itemModelIndex) {
        return register(name, ingredient, style, itemModelIndex, Map.of());
    }

    private static ResourceKey<TrimMaterial> register(
        String name,
        Item ingredient,
        Style style,
        ...
        return REGISTRIES.resource(
            name,
            (context, key) -> TrimMaterial.create(
                name,
                ingredient,
                itemModelIndex,
                Component.translatable(Util.makeDescriptionId("trim_material", key)).withStyle(style),
