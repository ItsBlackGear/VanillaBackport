...
public class ShelfMushroomDecorator extends TreeDecorator {
    public static final MapCodec<ShelfMushroomDecorator> CODEC = Codec.floatRange(0.0F, 1.0F)
        .fieldOf("probability")
        .xmap(ShelfMushroomDecorator::new, decorator -> decorator.placementProbability);
    ...
    private static boolean tryPlaceMushroomOnStandingTree(
        final TreeDecorator.Context context, BlockPos logPos, Direction facing, RandomSource random
    ) {
    ...
    private static boolean checkBlock(TreeDecorator.Context context, BlockPos pos, Predicate<BlockState> predicate) {
        return context.level().isStateAtPosition(pos, predicate);
    }
    
    public static boolean isReplaceable(TreeDecorator.Context context, BlockPos pos) {
        return checkBlock(context, pos, BlockBehaviour.BlockStateBase::canBeReplaced);
    }
    
    public static boolean isWaterOrWaterNearby(TreeDecorator.Context context, BlockPos pos) {
