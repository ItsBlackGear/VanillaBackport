...
public class CowVariants {
    ...
    public static final ResourceKey<CowVariant> TEMPERATE = register(
        "temperate",
        CowVariant.ModelType.NORMAL,
        "cow",
        SpawnPrioritySelectors.fallback(0)
    );
    public static final ResourceKey<CowVariant> WARM = register(
        "warm",
        CowVariant.ModelType.WARM,
        "warm_cow",
        ModBiomeTags.SPAWNS_WARM_VARIANT_FARM_ANIMALS
    );
    public static final ResourceKey<CowVariant> COLD = register(
        "cold",
    ...
    private static ResourceKey<CowVariant> register(String key, CowVariant.ModelType type, String adultAssetId, TagKey<Biome> biome) {
        return register(key, type, adultAssetId, SpawnPrioritySelectors.single(new RawBiomeCheck(biome), 1));
    }

    private static ResourceKey<CowVariant> register(String key, CowVariant.ModelType type, String adultAssetId, SpawnPrioritySelectors selectors) {
        ResourceLocation adultTexture = VanillaBackport.vanilla("entity/cow/" + adultAssetId);
        return REGISTRY.resource(key, new CowVariant(new ModelAndTexture<>(type, adultTexture), selectors));
    }
}