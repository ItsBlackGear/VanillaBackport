...
public abstract class TrackedWaypoint implements Waypoint {
    ...
    private void write(ByteBuf buf) {
        ...
        BufferUtils.writeEither(byteBuf, this.identifier, UUIDUtil.STREAM_CODEC, FriendlyByteBuf::writeUtf);
        Icon.STREAM_CODEC.encode(byteBuf, this.icon);
        byteBuf.writeEnum(this.type);
        this.writeContents(buf);
    }
    
    protected abstract void writeContents(ByteBuf buf);
    
    private static TrackedWaypoint read(ByteBuf buf) {
        FriendlyByteBuf byteBuf = new FriendlyByteBuf(buf);
        Either<UUID, String> identifier = BufferUtils.readEither(byteBuf, UUIDUtil.STREAM_CODEC, FriendlyByteBuf::readUtf);
        Icon icon = Icon.STREAM_CODEC.decode(byteBuf);
    ...
    private static class AzimuthWaypoint extends TrackedWaypoint {
        ...
        @Override
        protected void writeContents(ByteBuf buf) {
