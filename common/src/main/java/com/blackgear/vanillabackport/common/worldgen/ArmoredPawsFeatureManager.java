    ...
    public void bootstrap() {
        this.addIf(VanillaBackport.COMMON_CONFIG.hasArmadillos.get(), (context, writer) -> {
            this.getOrCreateBiomeBuilder(ModBiomeTags.SPAWNS_ARMADILLOS_FREQUENTLY)
                .add(() -> writer.addSpawn(MobCategory.CREATURE, new MobSpawnSettings.SpawnerData(ModEntityTypes.ARMADILLO.get(), 10, 2, 3)));

            this.getOrCreateBiomeBuilder(ModBiomeTags.SPAWNS_ARMADILLOS)
                .add(() -> writer.addSpawn(MobCategory.CREATURE, new MobSpawnSettings.SpawnerData(ModEntityTypes.ARMADILLO.get(), 6, 1, 2)));
        });
