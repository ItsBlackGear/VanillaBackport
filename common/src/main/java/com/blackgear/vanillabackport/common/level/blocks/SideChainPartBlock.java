...
public interface SideChainPartBlock {
    ...
    default List<BlockPos> getAllBlocksConnectedTo(LevelAccessor level, BlockPos pos) {
        ...
        if (!this.isConnectable(state)) {
            ...
            results.add(pos);
            this.addBlocksConnectingTowards(neighbors::left, SideChainPart.LEFT, posx -> results.add(0, posx));
            this.addBlocksConnectingTowards(neighbors::right, SideChainPart.RIGHT, results::add);
            return results;
        }
    }
