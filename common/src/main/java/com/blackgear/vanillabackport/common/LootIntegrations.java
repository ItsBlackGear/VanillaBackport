...
public class LootIntegrations implements LootModifier.LootTableModifier {
    ...
    public void modify(ResourceKey<LootTable> key, LootModifier.LootTableContext context, boolean builtin) {
        ...
        if (key.equals(BuiltInLootTables.WOODLAND_MANSION) && VanillaBackport.COMMON_CONFIG.hasResin.get()) {
            context.addToPool(
                1,
        ...
        if (CONTAIN_BUNDLE.contains(key) && VanillaBackport.COMMON_CONFIG.hasVillageBundles.get()) {
            context.addPool(
                LootPool.lootPool()
                    .setRolls(ConstantValue.exactly(1.0F))
